    ...
    const OrderTimeLine = ({ createdAt, currentState }) => {
        return (
            <Timeline align={"left"}>
                ...
                {currentState <= 6 && (
                    <Timeline.Item
                        className={classNames(classes.oldTimeItem, {
                            [classes.newTimeItem]: currentState === 6,
                        })}
                    >
                        <>
                            <p>{Moment2(createdAt).format("MMMM DD,YYYY")}</p>
                ...
                {currentState <= 5 && (
                    <Timeline.Item
                        className={classNames(classes.oldTimeItem, {
                            [classes.newTimeItem]: currentState === 5,
                        })}
                    >
                        <p>
                ...
                {currentState <= 4 && (
                    <Timeline.Item
                        className={classNames(classes.oldTimeItem, {
                            [classes.newTimeItem]: currentState === 4,
                        })}
                    >
                        <p>
                            <Moment format="MMMM DD,YYYY" add={{ days: 5 }}>
                ...
                {currentState <= 3 && (
                    <Timeline.Item
                        className={classNames(classes.oldTimeItem, {
                            [classes.newTimeItem]: currentState === 3,
                        })}
                    >
                        <p>
                            <Moment format="MMMM DD,YYYY" add={{ days: 5 }}>
                ...
                {currentState <= 2 && (
                    <Timeline.Item
                        className={classNames(classes.oldTimeItem, {
                            [classes.newTimeItem]: currentState === 2,
                        })}
                    >
                        <p>
                            <Moment format="MMMM DD,YYYY" add={{ days: 5 }}>
                ...
                {currentState <= 1 && (
                    <Timeline.Item
                        className={classNames(classes.oldTimeItem, {
                            [classes.newTimeItem]: currentState === 1,
                        })}
                    >
                        <p>
                            <Moment format="MMMM DD,YYYY" add={{ days: 5 }}>
    ...
    return (
        <Card chart className={classes.addCursor}>
            <CardHeader
                color="success"
                style={{
                    backgroundImage: `linear-gradient(40deg, ${pickRandGradient()})`,
                    textAlign: "center",
                }}
            >
                <p style={{ fontSize: "1.4em" }}>
                    Ordered by {order.user.name} from {order.shop.name}
                </p>
                <OrderTimeLine
                    createdAt={order.createdAt}
                    currentState={transformCurrentStateToNum(
                        order.currentState
                    )}
                />
            </CardHeader>
            <CardBody>
                <h6 style={{ fontSize: "1.3em" }}>
                    Product Ordered - {order.product.name}
                </h6>
                <p
                ...
                <Grid container spacing={1} style={{ marginTop: "1em" }}>
                    <Grid item xs={4} md={4} lg={5} style={{ margin: 0 }}>
                        <Button
                            color="tiki"
                            style={{ width: "90%", marginRight: "0.2em" }}
                            onClick={() => {
                                setCurrentOrder(order);
                                toggleOrderUpdateForm((val) => !val);
                            }}
                        >
                            Update Order
                        </Button>
            ...
            <CardFooter chart>
                <div className={classes.stats}>
                    <AccessTime />
                    <Moment fromNow style={{ textTransform: "capitalize" }}>
                        {shopsLastUpdated}
                    </Moment>
                </div>
            </CardFooter>
        ...
        if (isSearchByCurrentState === true) {
            switch (filterOptions2) {
                ...
                case "Tiki Received":
                    setOrders_(
                        allOrders.filter(
                            (order) => order.currentState === "Tiki Received"
                        )
                    );
                    setIsLoading(false);
                    return;
                case "Getting Product":
                    setOrders_(
                        allOrders.filter(
                            (order) => order.currentState === "Getting Product"
                        )
                    );
                    setIsLoading(false);
                    return;
                case "Packing":
                    setOrders_(
                        allOrders.filter(
                            (order) => order.currentState === "Packing"
                        )
                    );
                    setIsLoading(false);
                    return;
                case "Shipping":
                    setOrders_(
                        allOrders.filter(
                            (order) => order.currentState === "Shipping"
                        )
                    );
                    setIsLoading(false);
                    return;
            ...
            switch (filterOptions) {
                case "id":
                    setOrders_(
                        allOrders.filter((order) => order._id === inputText)
                    );
                    setIsLoading(false);
                    return;
                case "productName":
                    setOrders_(
                        allOrders.filter(
                            (order) => order.product.name === inputText
                        )
                    );
                    setIsLoading(false);
                    return;
                case "price":
                    setOrders_(
                        allOrders.filter(
                            (order) => order.total.toString() === inputText
                        )
                    );
                    setIsLoading(false);
                    return;
                case "quantity":
                    setOrders_(
                        allOrders.filter(
                            (order) => order.quantity.toString() === inputText
                        )
                    );
                    setIsLoading(false);
                    return;
        ...
        <div style={{ width: "100%" }}>
            ...
            >
                <Grid item xs={2} md={2} lg={1} style={{ margin: 0 }}>
                    ...
                    >
                        ...
                        >
                            Find Order to Update or Delete
                        </Button>
                    </section>
                </Grid>
                    ...
                    >
                        ...
                        >
                            <Button
                                ...
                                size={"sm"}
                                className={classes.title2}
                                onClick={() => setToggleList2((val) => !val)}
                            >
                                Filter by {filterOptions2}{" "}
                                {toggleList2 ? (
                                    <ExpandLess
                                        style={{ marginLeft: "0.5em" }}
                                    />
                            ...
                            <List
                                component="nav"
                                aria-label="filter options2"
                                className={classNames(classes.listStyle, {
                                    [classes.showList]: toggleList2,
                                })}
                                style={{ marginTop: "1em" }}
                            >
                                <ListItem
                                ...
                                <ListItem
                                    ...
                                    selected={
                                        "Getting Product" === filterOptions2
                                    }
                                >
            ...
            <Grid container spacing={2} style={{ marginLeft: "0.5em" }}>
                {orders_ !== null && orders_.length > 0 && !showUpdateForm
                    ? orders_.map((order, index) => (
                          ...
                          >
                              <OrderCard
                                  ...
                                  toggleOrderUpdateForm={setShowUpdateForm}
                                  isLoading={isLoading}
                    ...
                    : null}
                {orders_ !== null &&
                    orders_.length === 0 && ( // if search result is empty
                        <Grid
                            item
                            xs={10}
                            md={10}
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    No orders found with "{filterOptions2}".
                                </p>
                            </section>
                        </Grid>
                    )}
